...
import com.jeesite.common.shiro.realms.E;
...
public class AccountController extends BaseController{
	...
	public String register(){
		return "modules/sys/account/register";
	}

	@RequestMapping(value = "instructions")
	public String instructions(){
		return "modules/sys/account/instructions";
	}

	@PostMapping(value = "/saveUser")
	@ResponseBody
	public Object saveUser(@RequestParam  Map param){
		try {
			EmpUser empUser=new EmpUser();
			empUser.setUserName((String) param.get("userName"));
			empUser.setMobile((String) param.get("mobile"));
			empUser.setLoginCode((String) param.get("loginCode"));
			empUser.setPassword((String) param.get("pass"));
			empUser.setUserType("employee");
			String str=(String) param.get("loginCode")+"_"+IdGen.randomBase62(4).toLowerCase();
//			empUser.setUserCode((String) param.get("loginCode")+"_"+IdGen.randomBase62(4).toLowerCase());
			Employee employee = empUser.getEmployee();
			// 如果员工编码为空，则使用用户编码
			if (StringUtils.isBlank(employee.getEmpCode())){
				employee.setEmpCode(str);
			}
			// 如果员工工号为空，则使用员工编码
			if (StringUtils.isBlank(employee.getEmpNo())){
				employee.setEmpNo((String) param.get("loginCode"));
			}
			// 如果员工姓名为空，则使用昵称名
			if (StringUtils.isBlank(employee.getEmpName())){
				employee.setEmpName(empUser.getUserName());
			}
			// 2、保存用户
			empUser.setRefCode(employee.getEmpCode());
			empUser.setRefName(employee.getEmpName());
			userService.save(empUser);
			empUserService.updateBy(param.get("loginCode"),str);
			//默认角色为普通用户
			UserRole userRole=new UserRole();
			userRole.setUserCode(str);
			userRole.setRoleCode("1008611");//普通客户
			empUserService.saveUserRole(userRole);
			Office office=empUserService.selectByOffice("SD");//山东
			// 3、保存员工
			employee.setOffice(office);
			employee.setIsNewRecord(empUser.getIsNewRecord());
	...
	@ResponseBody
	public Object checkUser(@RequestParam Map params){
		//查询有没有这个用户
		int num=empUserService.selectByLoginCode(params.get("loginCode"));
		return num;
	}
	/**
